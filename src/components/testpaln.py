...
class TestPlan(BaseNode):
    ...
    async def before_hook(self):
        async with async_session() as session:
            ...
            # )
            # session.add(plan)
            u_sql = update(StrTestPlan).where(and_(
                StrTestPlan.suite_key == global_context.global_storage['task_key'],
                StrTestPlan.plan_key == global_context.global_storage['plan_key'],
