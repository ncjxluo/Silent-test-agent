            ...
            for key, value in local_ctx.items():
                if "_sampler" in key:
                    ...
                    if assert_res_sign == '整体断言:失败':
                        case_status = 1
                    case_step = {
                        "_index": "str_test_case_step",
                        "_source":{
                            "suite_key": global_context.global_storage['task_key'],
                            "plan_key": global_context.global_storage['plan_key'],
                            "case_key": case_id,
                            "step_id": step_id,
                            "step_name": key,
                            "user_variables": local_ctx.get("user_variable"),
                            ...
                            "real_response": json.dumps(local_ctx.get(key).get("last_response"), ensure_ascii=False),
                    ...
                    step_id = step_id + 1
            # print(f"case_steps:{case_steps}")
            await async_bulk(es, case_steps)
            case = StrTestCase(
                suite_key=global_context.global_storage['task_key'],
                plan_key=global_context.global_storage['plan_key'],
                case_key=case_id,
